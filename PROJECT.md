...
**The user chooses the input method: the clipboard, or a virtual keyboard.** The choice is made at first launch and can be changed from the menu bar at any time, and it is the one choice rather than two flags because the hotkey and the output come as a pair. The clipboard method needs nothing an administrator approves: its hotkey is a registered hot key, which needs neither Input Monitoring nor Accessibility, and the words go on the clipboard for the user to paste. That is what a managed work Mac can run, where macOS asks for an administrator to grant Accessibility or Input Monitoring. The virtual keyboard method is the one below, and choosing it registers the helper and shows what it still needs. An earlier version of this decision made the virtual keyboard the only path; a Mac where nobody wants to approve a driver extension is why it no longer is.
...
**The virtual keyboard's hotkey is a session-level `CGEventTap`; the clipboard's is a registered hot key.** Either way, a press shorter than about 250 ms is a tap and toggles listening, and anything longer is push-to-talk. Right Option is the virtual keyboard's default key. Fn/Globe is configurable but macOS claims it by default, so it needs a System Settings change. A registered hot key cannot be a modifier alone and does not tell left from right, so the clipboard's default is Control+Shift+D, clear of Control+Option, which is VoiceOver's.

**What it needs depends on the input method, and it cannot be sandboxed.** The clipboard needs the microphone and nothing else. The virtual keyboard needs Microphone, Accessibility, and Input Monitoring, and beyond those the user approves the driver extension and the helper login item, once each. Event taps and a root helper reached over XPC are incompatible with the App Store sandbox, so the app is Developer ID signed, notarized, and distributed outside the store.
