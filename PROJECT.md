...
**The user chooses the delivery: the clipboard, or a virtual keyboard.** The choice is made at first launch and can be changed from the menu bar at any time, and it is the one choice rather than two flags because the hotkey and the output come as a pair. The clipboard delivery needs nothing an administrator approves: its hotkey is a registered hot key, which needs no permission, and the words go on the clipboard for the user to paste. That is what a managed work Mac can run, where macOS asks for an administrator to grant Accessibility. The virtual keyboard delivery is the one below, and choosing it registers the helper and shows what it still needs. An earlier version of this decision made the virtual keyboard the only path; a Mac where nobody wants to approve a driver extension is why it no longer is.
