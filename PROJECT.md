...
**The virtual keyboard's hotkey is a session-level `CGEventTap`; the clipboard's is a registered hot key.** Either way, a press shorter than about 250 ms is a tap and toggles listening, and anything longer is push-to-talk. Right Option is the virtual keyboard's default key. Fn/Globe is configurable but macOS claims it by default, so it needs a System Settings change. A registered hot key cannot be a modifier alone and does not tell left from right, so the clipboard's default is Control+Option+D.
