...
@Suite struct ConfigReportTests {
    ...
    @Test func aRouteIntoAnAppThisMacDoesNotHaveIsNamed() throws {
        let config = try Config(toml: Self.slack)
    ...
    @Test func theReportReadsTheFileBack() throws {
        ...
        let config = try Config(toml: """
            ...
            chord = { modifiers = ["rightOption"] }
            """)
        let report = ConfigReport(.file(config, at: url), appExists: Self.noApps)
    ...
    @Test func aReportWithNoFileSaysThereIsNoFile() {
        ...
        let report = ConfigReport(.noFile(at: url), appExists: Self.noApps)
    ...
    @Test func aModeIsReadBackInTheWordsTheFileUses() throws {
        let report = ConfigReport(.file(try Config(toml: Self.slack), at: URL(filePath: "/tmp/x.toml")), appExists: { _ in true })
    ...
    @Test func theReportPrintsTheGapsItFound() throws {
        let config = try Config(toml: Self.slack)
        let report = ConfigReport(.file(config, at: URL(filePath: "/tmp/x.toml")), appExists: Self.noApps)
