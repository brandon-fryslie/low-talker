...
The package lands in a directory made for that run, inside one fixed directory under your temporary area that the CLI first proves you own. The run's directory and the package go when the run ends. No two runs share a file, so a build fetching the package cannot replace the bytes an install checked while it waits for `sudo`, and a download or copy that dies partway leaves no older bytes for the checksum to approve.
