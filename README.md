...
`lowtalker driver` does the work. The CLI each app carries has these verbs too ("The CLI without a checkout" above), so a Mac with no clone installs the driver with the same binary that reads it:

    lowtalker driver state                 # the readings to stderr, one verdict to stdout
    lowtalker driver expect <verdict>      # assert that state
    lowtalker driver install [package]     # download or take a carried copy, verify, install, activate
    lowtalker driver fetch <directory>     # download and verify the package, for a release to carry
    lowtalker driver check <package>       # verify a package file against the pins
    lowtalker driver remove                # deactivate, delete, forget receipt
...
`state` prints a fact table to stderr for a reader and one verdict word to stdout, so `$(lowtalker driver state)` is exactly the verdict. The verdicts are `absent`, `installed-inactive`, `awaiting-approval`, `disabled`, `enabled`, `running`, `pending-reboot`, `residue`, and `unknown`. `enabled` means macOS has the extension switched on; `running` means that and the driver has published its node in the IORegistry. `running` is the fully working state. `install` exits 0 when the driver is active and 2 when it waits for your approval; `remove` exits 0 when the driver is gone and 2 when only a restart is left; both exit 1 when they refuse, with the reason.

These verbs were a bash script in this repo, `scripts/virtual-hid-driver`, until the app began carrying the CLI. A script found its CLI at a checkout path and kept its own copies of the paths it deleted; now the paths `remove` deletes are the constants the probe detects by, in one process. The driver's identity, the package's version, checksum and signer, and the two payload trees are pinned in `Sources/DriverExtension`, and `make check-docs` reads `lowtalker driver pins` and fails when a value quoted in this file has drifted.
...
Two version numbers travel together and are not the same. The package is 8.4.0 and carries the Manager and Daemon helper apps; the driver extension inside it is 1.8.0, which is what `systemextensionsctl` reports. It has not moved across many package releases, so a package upgrade that leaves `systemextensionsctl` still reading 1.8.0 has not failed. `DriverPackage` pins both numbers and the package's SHA-256 checksum. This file quotes the versions, not the checksum, and `make check-docs` fails when a number quoted here disagrees with the pins.
...
### Installing
    ...
    lowtalker driver install
    lowtalker driver install /Applications/LowTalker.app/Contents/Resources/Karabiner-DriverKit-VirtualHIDDevice.pkg

With no argument the package is downloaded from GitHub. A release carries the same package in its bundle, so the second form installs with no network. Either way the package is checked before installing: against the pinned SHA-256, and against the signature, which must be `Developer ID Installer: Fumihiko Takayama (G43BCU2T37)` on a chain macOS trusts. A carried copy gets no trust for having shipped beside the app. It is copied into a directory of the CLI's own first and checked there, so whoever could write beside the app cannot swap the bytes between the check and `installer`.

The package lands in a directory made for that run, inside one fixed directory under your temporary area that the CLI first proves you own. The run's directory and the package go when the run ends, Ctrl-C included. No two runs share a file, so a build fetching the package cannot replace the bytes an install checked while it waits for `sudo`, and a download or copy that dies partway leaves no older bytes for the checksum to approve.
...
On a Mac that has never approved this driver, activation stops and waits for you, and `install` does not return until you have: the Manager holds the request open. Measured on studious.local over ssh, `install` downloaded, verified and installed the package, then sat in the Manager's `activate` for as long as nobody clicked. So it says what it waits for before it starts waiting:
...
Then `lowtalker driver expect enabled` confirms it. macOS remembers the approval per developer team and bundle identifier, so a Mac that has ever approved this driver activates silently on reinstall: the extension comes up already switched on, and the missing prompt is expected, not a skipped step.
...
On inferno.local (Mac16,6, macOS 26.5.1, System Integrity Protection enabled): `absent` was confirmed on a clean machine, when these verbs were still a bash script. The install has not been run there: it needs an administrator password typed at that machine, and the approval click above.
...
The steps that want the driver put right name `lowtalker driver install` (and `remove` before it, for `residue`), spelled as the full path of the CLI the reader has: the CLI's own when `lowtalker onboard` prints it, and the one inside the app's bundle when the menu shows it. Every reader has that binary, so there is one instruction rather than one for a clone and another for everyone else. `install` is two operations: `sudo installer` puts the payload down, and a separate `Karabiner-VirtualHIDDevice-Manager activate` — `/Applications/.Karabiner-VirtualHIDDevice-Manager.app/Contents/MacOS/Karabiner-VirtualHIDDevice-Manager` — asks macOS to register it. So one command moves a reader off `absent` and off `installed-inactive` alike. Both steps say to run it as the logged-in user and not under sudo, for the reason in the next section. The package itself is 8.4.0, at <https://github.com/pqrs-org/Karabiner-DriverKit-VirtualHIDDevice/releases/download/v8.4.0/Karabiner-DriverKit-VirtualHIDDevice-8.4.0.pkg>.
...
The second half is that the install could not be finished by root alone even so. Activation must be requested by the console user, because macOS attributes a driver-extension activation to whoever asks and the approval the user gives answers that request — which is why `lowtalker driver install` runs the Manager as the logged-in user and takes sudo only for the file steps. An install driven from the app would be root for one half and the user for the other, and the half that needs root is the half that cannot happen yet.
...
- `not registered` says to press Allow in the app's guided setup, which is the only place the app registers a helper that is not yet enabled.
...
| Input Monitoring | the hotkey source is the event tap | asks to read the keyboard (`CGRequestListenEventAccess`). Normally already met by then: once Accessibility is held, macOS answers Input Monitoring from it, so the walk passes this step without a dialog. The step shows only when Input Monitoring was switched off |
