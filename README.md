...
Verified on this Mac with the development copy: on the clipboard, a four-word utterance spoken during a held Control+Option+Command+D was on the pasteboard 698 ms after key-up, with no helper registration logged; switched to the virtual keyboard from the menu, the next utterance was typed into TextEdit 693 ms after key-up; and with the stored choice deleted, the next launch asked, and logged the answer. The alert that lists missing steps was not seen, since this Mac is missing none.
