...
On 2026-09-23 this was watched with the network off. The v0.1.0-alpha.3 image was downloaded from its release page, given a browser's quarantine, and opened with a packet filter blocking every outbound connection except DNS and the one API host the agent running the test talked to. `syspolicyd`'s ticket lookup at api.apple-cloudkit.com timed out, and Gatekeeper still allowed the mount and then the launch under its Notarized Developer ID rules. The app ran from `/Volumes/LowTalker` with no prompt, and `spctl` assessed both the image and the app as `source=Notarized Developer ID`. `xcrun stapler validate` is not an offline check: it asks CloudKit for the ticket and fails with error 68 when it cannot reach it, even on a stapled file.
