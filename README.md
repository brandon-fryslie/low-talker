...
A release carries its model. `scripts/sign-release` installs the default model into a store of its own and builds that store into the bundle at `Contents/Resources/model-store`, where the code signature seals it with the rest of the app. The app's first launch installs from it the way `--from <directory>` would. It rides inside the bundle rather than beside the app on the disk image, because dragging the app to Applications leaves behind anything that sat beside it. A bundle with no store, which is every development build, downloads from huggingface.co as before. A bundle whose store does not hold the model whole stops the load with the reason, and downloads nothing. Measured on 2026-09-16, the release bundle is 625 MB. Launched with no `low-talker` folder in Application Support and outbound network denied, it copied the model in 8 ms, a clone, since the bundle and Application Support were on one APFS volume; a copy off a disk image is a real one and was not measured. It was ready 164 s after launch, nearly all of that the Core ML load ("Load times and the Neural Engine cache" below), and put "Hello world, this is Low Talker." on the clipboard 679 ms after key-up. A carried weight file changed by one byte fails `codesign --verify`.
...
A release that carries its model still pays this on its first launch, because a model just installed has never been compiled here. Measured on 2026-09-16 with the Developer ID release, twice. Each time the release launched with no `low-talker` folder in Application Support and outbound network denied, so it installed from the store it carries. It was ready 164 s after launch the first time and 199 s the second, while the Mac was in use. On the second launch, a press released 6.5 s after launch waited for the model and put its words on the clipboard 200.6 s after launch. While the model is not yet ready, the menu bar icon is an hourglass, described to Accessibility as "LowTalker: preparing the model". The menu's model line is the one the log records, from "loading model, minutes the first time on this Mac, 0 s so far" to "ready (large-v3-v20240930_turbo_632MB) after 3 min 19 s", counted from launch, and while the wait lasts, a second line says a press made now is heard once the model is ready. A load that fails draws a warning triangle. These are this Mac's numbers. A Mac that has never run low-talker also runs Gatekeeper's first check of the notarized bundle, and that was not measured, since nothing is notarized yet (low-dist-idg.1).
...
The clipboard method leaves the words for you to paste, and a macOS **Service** lets one shortcut both end the dictation and put those words at your cursor, still without any grant the way ⌘V would need. The app declares a Service, "Insert LowTalker Dictation" ("Insert LowTalker Dev Dictation" for the development copy), under `NSServices` in `project.yml`; xcodegen writes it into the bundle's `Info.plist`, where Xcode merges it with the keys it synthesises. Its default shortcut is Command+Shift+2 for the release copy and Command+Shift+1 for the development one — the `serviceKey` `@` and `!` on a US layout — and you can rebind it under System Settings > Keyboard > Keyboard Shortcuts > Services > Text. A Service's shortcut wins only where the frontmost app has not bound the same keys itself.

When the app that is in front invokes the Service, it — not low-talker — inserts the returned text at its own cursor, so no key is posted and low-talker never reaches into it. The handler ends a tap-latched listening first, as a second tap of the chord would (`Hotkey.release`), waits out the sessions in flight up to `AppDelegate.serviceWait` by turning the main run loop rather than blocking it, and hands back the latest dictation's words. It says why, to the calling app and the log, when there is nothing to insert: the dictation is still being heard, nothing was heard, or no dictation has happened yet — never an empty or stale insert. Because the words are held until the next press begins, invoking the Service again re-inserts the same dictation.

Verified on this Mac with the development copy: a tap of Control+Shift+Command+D and the fixture `say/greeting`, with no second tap, left the listening latched; `NSPerformService("Insert LowTalker Dev Dictation")` then returned "Hello world, this is Low Talker." on the pasteboard it was handed in 1.5 s, the log reading `insert dictation: ended the listening, returned 33 characters` beside `heard 6 words 1345 ms after key-up`; a second call with nothing latched re-served the same words in 124 ms; and a freshly launched copy with no dictation refused the call, so the caller inserted nothing. Whether a bound shortcut reaches a real cursor is each app's to honour: iTerm2 and the native text apps carry text Services, while many Chromium and Electron apps carry none.
