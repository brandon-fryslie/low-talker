...
The two steps that want an install name two things, because they have two readers. Someone working from a clone runs `scripts/virtual-hid-driver install`, which fetches the pinned package, checks its checksum and its signature, installs it and activates it. Someone who installed LowTalker.app has no `scripts/` directory, so those steps also name the package itself — 8.4.0, at <https://github.com/pqrs-org/Karabiner-DriverKit-VirtualHIDDevice/releases/download/v8.4.0/Karabiner-DriverKit-VirtualHIDDevice-8.4.0.pkg>. The version and URL live in `DriverPackage` in Swift so onboarding can say them; the script keeps the pin it acts on, because it is the file that fetches and checksums the bytes, and `make check-docs` fails when the two copies disagree.
...
The Keyboard Setup Assistant's row reads whether `/Library/Preferences/com.apple.keyboardtype.plist` already holds an answer under the key `10203-5824-0`, this virtual keyboard's product-vendor-country. The file is world-readable, so reading it needs no privilege. Writing it does, and that is the keyboard helper's job as it starts, described above — so this row is the only one whose step names nothing to run. It says which row it is waiting behind: the answer is filed once the keyboard helper is answering, and until then the row reads `will ask on first use`. The reader and the writer take the file and the key from one place, `VirtualKeyboardIdentity`, so the process that writes the answer and the process that reads it back cannot come to mean different files.

Both onboarding surfaces name the file and the key nowhere in their steps, which is deliberate and is checked: `RequirementTests` fails if the step ever regrows `sudo`, `defaults`, the domain or the key.
