...
prints everything that must hold before low-talker can type, read off this Mac now, with the step for whatever is missing indented under it. On this Mac today:
    ...
    Keyboard helper: registered, but another job holds the service
...
- `registered, but another job holds the service` names `com.lowtalker.keyboardd.dev` as the development job and says to run `scripts/keyboard-helper uninstall`, then launch LowTalker again.
...
The second of those is the case the app could not see before, and it is what this Mac is in right now: `SMAppService.status` returns `.enabled`, raw value 1, while the dev job `com.lowtalker.keyboardd.dev` holds the Mach service. `lowtalker onboard` and the menu both read `Keyboard helper: registered, but another job holds the service`, name the dev job, and give the command that removes it, which is the output above.
