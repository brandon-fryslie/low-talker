...
`clickElement` searches the frontmost app's Accessibility tree breadth first for an element of that role and title, stopping at 2000 elements, and clicks the centre of its frame; an element with no area is refused by name. The other two forms are `{"click":{"at":{"x":697.5,"y":475},"button":"left","times":1}}` and `{"scroll":{"at":{"x":100,"y":200},"vertical":-3,"horizontal":0}}`. A scroll of 0 by 0 is a bare move, which is how the loop is measured: `[{"scroll":{"at":{"x":1400,"y":800},"vertical":0,"horizontal":0}}]` through the same `act`, with the context's `frontmostApp` set to whatever is in front.

Measured on Brandon's MacBook Pro (Mac14,5, macOS 26.3, display 1512 by 982 points) on 2026-09-07. The dialog was raised with `osascript -e 'do shell script "true" with administrator privileges' &`, which puts a com.apple.SecurityAgent authentication dialog in front; its Cancel button's Accessibility frame was (641, 460, 113, 30). From the cursor at (784, 141), `act` clicked the frame's centre (697.5, 475) in 4 move reports, 58 ms from the actions being handed over to the click's acknowledgement, the Accessibility search included; the window was gone afterwards and osascript exited with "User canceled. (-128)". The whole `act` process took about 300 ms wall, most of it process start and the 100 ms the raise step sleeps. Bare moves, cursor read back afterwards: to (100, 100) in 38 ms, landing at (99.74, 100.40); to (1400, 800) in 60 ms, at (1400.00, 800.05); to (756, 450) in 20 ms, at (756.16, 449.80). Every landing was within half a point.

The mouse exists because macOS protects its consent and approval dialogs from synthetic events, which is what onboarding's driver-extension and login-item approvals (ticket low-keyboard-3ti.7) will meet, and the virtual mouse is hardware to macOS. That property was not demonstrated here. A synthetic click, three CGEvents (moved, left down, left up) posted at the HID event tap from a Terminal process allowed under Accessibility, also dismissed that SecurityAgent Cancel: the window count went to 0 and osascript exited "User canceled" before the virtual mouse was tried. So that dialog does not ignore synthetic events on this Mac. An attempt to raise a TCC consent alert from a throwaway app got no alert either: an Accessibility request opens System Settings directly on this macOS, a Desktop-folder read from an app launched with `open` inherited Terminal's grants, and an Apple Events request from a launchd-launched copy was denied with no prompt. The place the property will actually be met is onboarding's approvals.
