...
The status menu lists both under "Delivery" with the current one checked, and choosing the other takes effect at once: a press still open is ended, sessions in flight finish, and a loop with the new delivery comes up behind the same hotkey. Choosing the virtual keyboard registers the helper, and when the driver, the helper or Keyboard Setup Assistant still needs something, an alert lists the steps. Choosing the input method installs it: the app copies its input method into `~/Library/Input Methods`, registers it, switches it on and selects it, and an alert says what is left when anything is ("The input method's row" below). On a Mac that has never had it, that is a logout: macOS switches on an input method first installed during a login session only after the next login, and the app switches it on the next time it is opened after that. Choosing the delivery already checked repairs it when it reads as not ready, which is how an input method deleted by hand is put back. A delivery that reads ready is left alone, along with any press it is holding open. The choice is kept per installation in its defaults, under `inputMethod` — the spelling from before the rename, kept because the word on disk is every installed copy's stored answer; `defaults delete ai.promptctl.low-talker.dev inputMethod` makes the development copy ask again at its next launch.
...
### The input method's row

The input method delivery needs no grant and no approval, so it has one requirement: where this installation's input method stands with the text input system, read every time the menu opens and never kept, so a bundle deleted from `~/Library/Input Methods` by hand reads as missing at the next open. The text input system lists two sources for the bundle: the input method, under its bundle identifier, and its one mode, under that identifier plus `.text`. Switched on is read off the first, because a mode reads as enabled whenever its bundle is registered. Measured on 2026-09-24, Apple's Ainu and Kotoeri modes did so on a Mac where neither was switched on. Selected is read off the second.

So the input method's row reads one of five things:

- `selected` means it is the input source in use, and there is nothing to do.
- `enabled, not selected` means another source is in use. The step says to choose Input Method under Delivery again, and that no input method can be selected while an app holds secure keyboard entry.
- `registered, switched off` is where a first install waits for the next login, and also what removing the input method in System Settings reads as. The step says to choose the delivery again first, which undoes the removal. If it still reads switched off, it says to log out and back in and then open the app, since nothing opens it at login.
- `installed, not registered` says to choose Input Method under Delivery, which registers it.
- `not installed` says the same, and choosing it copies the bundle back in before registering it. A copy from another build, or one damaged since, reads as this too.

Why a logout, measured on 2026-09-24 on a Mac that had never registered this input method (macOS 15.0.1, low-input-method-s71.ssn). The bundle was copied in mid-session. It carried a version and a Developer ID signature, and it registered. Then `TISEnableInputSource` answered `noErr` and changed nothing. Writing it into `AppleEnabledInputSources` by hand did not switch it on either. Neither did restarting TextInputSwitcher, TextInputMenuAgent and imklaunchagent, posting the enabled-sources-changed notifications, or registering again. In the same session Apple's Ainu switched on at the first call. So the app says it at the moment of the install: the status line reads `off — the input method is waiting for your next login: macOS registered … and will not switch it on until you log out and back in`, and the alert shows this row. Any other refusal reads `off — the input method could not be installed: …`.
